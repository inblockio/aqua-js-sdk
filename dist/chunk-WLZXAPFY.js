...
async function checkInternetConnection() {
  try {
    const { checkInternetConnectivity } = await import("./platform-ZEZNSY5V.js");
  ...
  switch (revisionType) {
    ...
    case "form":
      ...
      for (let key of formDataSortedKeys) {
        ...
        let value = formValue;
        console.log(`formValue ${formValue} type ${typeof formValue}`);
...
var MetaMaskSigner = class {
  ...
  */
  async signInNode(verificationHash) {
    const { createHttpServer: createHttpServer2 } = await import("./platform-ZEZNSY5V.js");
    const message = this.createMessage(verificationHash);
  ...
  async signInReactNative(verificationHash, network) {
    ...
    return new Promise((resolve, reject) => {
      ...
      global.__aquaMetaMaskResolve = async (signature, address) => {
        ...
        try {
          const cleanedAddress = ethers2.getAddress(address);
          const publicKey = await this.recoverPublicKey(message, signature);
