...
async function checkInternetConnection() {
  try {
    const { checkInternetConnectivity } = await import("./platform-CYOJTPRX.js");
...
var MetaMaskSigner = class {
  ...
  */
  async signInNode(verificationHash) {
    const { createHttpServer: createHttpServer2 } = await import("./platform-CYOJTPRX.js");
    const message = this.createMessage(verificationHash);
...
var P12Signer = class {
  ...
  async verify(signature, pubKey, data) {
    const { verify: cryptoVerify } = await this.getCrypto();
