...
export function maybeUpdateFileIndex(aquaObject, verificationData, revisionType, aquaFileName, formFileName) {
...
export function dict2Leaves(obj) {
    return Object.keys(obj)
        .sort() // MUST be sorted for deterministic Merkle tree
        .map((key) => getHashSum(`${key}:${obj[key]}`));
}
export function getFileHashSum(fileContent) {
    return getHashSum(fileContent);
}
export function getHashSum(data) {
    return createHash('sha256').update(data).digest('hex');
}
export function createNewAquaObject() {
    return {
        revisions: {},
        file_index: {},
        tree: {},
...
export function checkFileHashAlreadyNotarized(fileHash, aquaObject) {
    // Check if this file hash already exists in any revision
    const existingRevision = Object.values(aquaObject.revisions).find((revision) => revision.file_hash && revision.file_hash === fileHash);
    if (existingRevision) {
        return true;
    }
    else {
...
export const getLatestVH = (aquaObject) => {
