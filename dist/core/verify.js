...
export async function verifyAquaObjectUtil(aquaObject) {
    let logs = [];
    ...
    for (let revisionItemHash in verificationHashes) {
        let revision = aquaObject.revisions[revisionItemHash];
...
async function verifyRevision(revision, isScalar) {
    if (isScalar) {
    ...
    switch (input.revision_type) {
        ...
        case "signature":
            // Verify signature
            [typeOk, _] = await verifySignature(input, input.previous_verification_hash);
            break;
        case "witness":
            // Verify witness
            const [witnessStatus, witnessResult] = await verifyWitness(input, input.previous_verification_hash, doVerifyMerkleProof);
